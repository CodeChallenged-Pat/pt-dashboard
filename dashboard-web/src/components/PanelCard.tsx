...
export default function PanelCard({ config, onEdit, onRemove }: Props) {
  const { priority, title, width, height, cornerRadius, color, content } = config;

  // Compute a lighter version of the color for the glow
  const glowColor = color + "33"; // 20% opacity

  return (
    <div
      className="relative group transition-all duration-300 ease-in-out flex-shrink-0"
      style={{
        width: `${width}px`,
        height: `${height}px`,
        minWidth: "160px",
    ...
    >
      {/* Glow effect */}
      <div
        className="absolute inset-0 blur-xl opacity-30 transition-opacity group-hover:opacity-50 rounded-inherit"
        style={{
